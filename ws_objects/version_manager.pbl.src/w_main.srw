...
type tabpage_project from userobject within tab_1
end type
type sle_project_default from singlelineedit within tabpage_project
end type
type st_project_default from statictext within tabpage_project
end type
type sle_project_target from singlelineedit within tabpage_project
end type
type st_project_target from statictext within tabpage_project
end type
type sle_project_workspace from singlelineedit within tabpage_project
end type
type st_project_workspace from statictext within tabpage_project
end type
type tabpage_project from userobject within tab_1
sle_project_default sle_project_default
st_project_default st_project_default
sle_project_target sle_project_target
st_project_target st_project_target
sle_project_workspace sle_project_workspace
st_project_workspace st_project_workspace
end type
type tab_1 from tab within w_main
tabpage_svc tabpage_svc
tabpage_svc_2 tabpage_svc_2
tabpage_buildjob tabpage_buildjob
tabpage_project tabpage_project
end type
...
string title = "Version Manager PB2021"
...
string		is_DefaultProject
string		is_DefaultProjectLib
string		is_Workspace
string		is_DefaultTarget
string		is_LibList[]
...
public subroutine of_set_project_info ()
public function integer of_get_project_info ()
end prototypes
...
ls_repository = sle_repository.text
ii_limit = integer(ProfileString( ls_repository, 'CONTENT','Limit', '0'))
...
for li_i = 1 to ii_limit
	ls_application = profilestring( ls_repository, 'CONTENT', string(li_i), 'Missing Application Entry !')
...
public function integer of_set_svc_info ();boolean	lb_MergingRefreshPbl
string		ls_System
string 	ls_git
string 	ls_svn
string 	ls_vss
...
public subroutine of_set_project_info ();tab_1.tabpage_project.sle_project_workspace.text = is_workspace
tab_1.tabpage_project.sle_project_target.text = is_defaulttarget
tab_1.tabpage_project.sle_project_default.text = is_defaultprojectlib + ':' + is_defaultproject
end subroutine

public function integer of_get_project_info ();integer	li_i
integer	li_limit
long		ll_pos
string		ls_dummy[]
string		ls_tmp[]
...
if isnull( is_deployfilepath) or len(trim(is_deployfilepath)) = 0 then
	if Getfolder( "Select folder where " + is_application + " Workspace & Target files can be found", is_deployfilepath) < 1 then return -1
end if

ls_filespec = is_deployfilepath + "\" + "*.pbw"
li_limit = lnv_file.of_getfilelist( ls_filespec , ls_tmp, this)
if li_limit < 1 then return -1
is_workspace = is_deployfilepath + "\" + ls_tmp[1]

ls_tmp = ls_dummy
li_limit = lnv_file.of_parsefile( is_workspace, "DefaultTarget", '"', ls_tmp)
if li_limit < 1 then return -1

is_defaulttarget =is_deployfilepath + "\" + ls_tmp[1]

ls_tmp = ls_dummy
li_limit = lnv_file.of_parsefile( is_defaulttarget, "LibList", '"', ls_tmp)
if li_limit < 1 then return -1

li_limit = lnv_string.of_parsetoarray( ls_tmp[1], ";", is_LibList )

for li_i = 1 to li_limit
	ls_rc = librarydirectoryex( is_liblist[li_i], DirProject!)
	ls_rc = left(ls_rc, pos(ls_rc, "~t") - 1)
	ls_rc = lnv_string.of_removenonprint( ls_rc )
	if lnv_string.of_isempty( ls_rc ) = false then exit
next

is_defaultprojectlib = is_liblist[li_i]
is_defaultproject = ls_rc
...
string text = "Get Files For Deployment"
boolean cancel = true
end type
...
this.tabpage_project=create tabpage_project
this.Control[]={this.tabpage_svc,&
this.tabpage_svc_2,&
this.tabpage_buildjob,&
this.tabpage_project}
end on
...
on tabpage_project.create
this.sle_project_default=create sle_project_default
this.st_project_default=create st_project_default
this.sle_project_target=create sle_project_target
this.st_project_target=create st_project_target
this.sle_project_workspace=create sle_project_workspace
this.st_project_workspace=create st_project_workspace
this.Control[]={this.sle_project_default,&
this.st_project_default,&
this.sle_project_target,&
this.st_project_target,&
this.sle_project_workspace,&
this.st_project_workspace}
end on

on tabpage_project.destroy
destroy(this.sle_project_default)
destroy(this.st_project_default)
destroy(this.sle_project_target)
destroy(this.st_project_target)
destroy(this.sle_project_workspace)
destroy(this.st_project_workspace)
end on
